...
exports.run = async(client, message, args, ayar, emoji) => {
  ...
  if (!message.member.hasPermission(config.Dark)) return message.channel.send(embed.setDescription("Bu komutu kullanabilmek için **Dark** iznine sahip olmalısın!")).then(x => x.delete({timeout: 5000}));
